...
type
  TGUIXTestRunner = class(TForm, ITestLogger)
    ...
    txaProt: TMemo;
    lbTFVersion: TLabel;
    PanelBottomForFailDetail: TPanel;
    PanelRight: TPanel;
...
var
  fixture, tmpFixture : ITestFixture;
  test : ITest;
...
begin
  ...
  try
    //DAV>>>
    fixtureList.Sort(TComparer<ITestFixture>.Construct(
      function (const L, R: ITestFixture): Integer
      begin
    ...
    begin
      ...
      if (fixture.Name[1] = 'u') and fixture.HasChildFixtures then
        tmpFixture := fixture.Children.Items[0]
      else
        tmpFixture := fixture;

      fixtureNode := CreateNode(TestTree, tmpFixture.Name, tmpFixture.FullName);
      ...
      if fixture.HasChildFixtures then
        BuildTree(fixtureNode, tmpFixture.Children);
      for test in tmpFixture.Tests do
      begin
...
begin
 if logType = TLogLevel.Information then
   txaProt.Lines.Add('Information: ' + msg)
 else if logType = TLogLevel.Warning then
   txaProt.Lines.Add('Warning: ' + msg)
 else if logType = TLogLevel.Error then
   txaProt.Lines.Add('Error: ' + msg);
end;
