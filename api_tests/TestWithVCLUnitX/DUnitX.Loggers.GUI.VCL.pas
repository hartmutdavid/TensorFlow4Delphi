...
{$ENDIF}
  ...
  DUnitX.Loggers.GUI.VCL.RichEdit, System.ImageList, System.Actions, ATBinHex;
...
type
  ...
  var
    ...
    FRunning : boolean;
    //DAV>>>
    m_oATBinHex: TATBinHex;
    m_pStream: TMemoryStream;
    procedure HexProtBegin;
    procedure HexProtEnd;
    //DAV<<<
    procedure LoadTests;
  ...
  protected
    ...
    //DAV>>>
  public
    procedure MenuItemInterpreteDataClick(Sender: TObject);
    procedure HexProtWriteDesciption(i_sText: AnsiString);
    procedure HexProtWriteString(i_sText: AnsiString);
    procedure HexProtWriteChars(const i_pChars: PAnsiChar; i_iSize: Integer);
    //DAV<<<
  end;
...
var
 ...
 //DAV>>> Trenner
 g_aTrennBytes1: TBytes =[$3C,$7C,$3E];
 g_aTrennBytes2: TBytes =[$7C];
 //DAV<<<
...
//DAV>>>
procedure TGUIVCLTestRunner.HexProtBegin;
begin
 if Assigned(m_pStream) then
   m_pStream.Free;
 m_pStream := TMemoryStream.Create;
end;

procedure TGUIVCLTestRunner.HexProtEnd;
begin
 if Assigned(m_pStream) then begin
   m_oATBinHex.OpenStream(m_pStream, True);
   //-- m_pStream.Free;     Wird nicht hier geschlossen, da ein Kopieren im Hex-Viewer sonst nicht möglich ist.
   //-- m_pStream := Nil;
 end;
end;

procedure TGUIVCLTestRunner.HexProtWriteDesciption(i_sText: AnsiString);
var
 lng: Integer;
 l_sStr:   AnsiString;
begin
 if Assigned(m_pStream) then begin
   l_sStr := '<<' + i_sText + '>>';
   lng := Length(l_sStr);
   m_pStream.WriteBuffer(Pointer(l_sStr)^, lng);
 end;
end;

procedure TGUIVCLTestRunner.HexProtWriteString(i_sText: AnsiString);
var
 lng: Integer;
 n: UInt32;
begin
 if Assigned(m_pStream) then begin
   m_pStream.WriteBuffer(g_aTrennBytes1,Length(g_aTrennBytes1));
   n := UInt32(Length(i_sText));
   m_pStream.Write(n, sizeof(UInt32));
   m_pStream.WriteBuffer(g_aTrennBytes2,Length(g_aTrennBytes2));
   lng := Length(i_sText);
   m_pStream.WriteBuffer(Pointer(i_sText)^, lng);
 end;
end;

procedure TGUIVCLTestRunner.HexProtWriteChars(const i_pChars: PAnsiChar; i_iSize: Integer);
var
 i: Integer;
 n: UInt32;
 l_pCh: PAnsiChar;
...
begin
 if Assigned(m_pStream) then begin
   ...
   m_pStream.Write(n, sizeof(UInt32));
   m_pStream.WriteBuffer(g_aTrennBytes2,Length(g_aTrennBytes2));
   SetLength(l_aBytes,i_iSize);
   l_pCh := i_pChars;
   for i := 0 to i_iSize-1 do begin
     l_aBytes[i] := Byte(l_pCh^);
     Inc(l_pCh);
   end;
   m_pStream.WriteBuffer(l_aBytes,i_iSize);
 end;
end;
//DAV<<<
...
procedure TGUIVCLTestRunner.MenuItemInterpreteDataClick(Sender: TObject);
var
 l_sStr: String;
...
begin
 //-- Clipboard.Clear;
 m_oATBinHex.CopyToClipboard(True);
 l_sStr := Clipboard.AsText;
 if Length(l_sStr) > 0 then begin
   if not FormDataInterpreter.Visible then
     FormDataInterpreter.Visible := True;
   FormDataInterpreter.SetHexText(l_sStr);
 end;
end;

procedure TGUIVCLTestRunner.FormCreate(Sender: TObject);
var
 l_oMenuItem:  TMenuItem;
 l_oPopupMenu: TPopupMenu;
begin
 System.ReportMemoryLeaksOnShutdown := True;
 //DAV>>>
 m_pStream := Nil;
 m_oATBinHex := TATBinHex.Create(self);
 m_oATBinHex.Parent:= self.tabHex;
 m_oATBinHex.Align := alClient;
 m_oATBinHex.Mode  := vbModeHex;

 l_oPopupMenu := m_oATBinHex.PopupMenu;
 l_oMenuItem  := TMenuItem.Create(m_oATBinHex);
 with l_oMenuItem do begin
   Caption := 'Interprete Data';
   OnClick := MenuItemInterpreteDataClick;
 end;
 l_oPopupMenu.Items.Add(l_oMenuItem);
 //DAV<<<
...
procedure TGUIVCLTestRunner.miCopySelectedClick(Sender: TObject);
begin
 Clipboard.AsText := rchText.SelText;
end;
